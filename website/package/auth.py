    ...
    if request.method=="POST":
        ...
        if user:
            if check_password_hash(user.password,password):
                ...
                return redirect(url_for('views.Data_stewards'))
            else :
...
def sign_up():
    if request.method=="POST":
        ...
        if user:
            ...
            flash('Email needs to be more than 4 character', category= 'error')
        elif len(firstname)<2:
            flash('First name need more than 2 characters', category= 'error')
